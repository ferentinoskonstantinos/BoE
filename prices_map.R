...
# that shows the average price in each region in our final sample
...
# We construct a heat map of the mean price in each region in our sample, 
...
price_NUTS<-aggregate(data_NUTS$Price, by=list(NUTS118NM=data_NUTS$NUTS118NM), FUN=mean)
price_NUTS
colnames(price_NUTS)[2]<-"Mean_Price"
...
NUTS<-join(price_NUTS, test, by='NUTS118NM')
NUTS
colnames(NUTS)[3]<-"id"
...
rm(codes, price_NUTS, NUTS, test)

# We construct the heat map of England and Wales with the mean house prices 
